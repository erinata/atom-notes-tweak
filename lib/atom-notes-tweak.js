...
export default {
   ...
   */
  activate (state) {
    console.info('Activating atom-notes-tweak')
    this.store = null
  ...
  handleURI (parsedUri) {
    if (parsedUri.pathname === '/toggle') {
      ...
      atom.notifications.addError(
        'Only the URI `atom://atom-notes-tweak/toggle` is currently supported')
    }
  },
  ...
  serialize () {
    ...
    if (this.store && this.doSerialize) {
      ...
      try {
        ...
        console.error('atom-notes-tweak: serialization failure', e)
      }
...
function makeReady (self, state = null) {
  setTimeout(() => {
    if (!ensureNotesDirectory()) {
      ...
      __guard__(atom.packages.getActivePackage('atom-notes-tweak'), x => x.deactivate())
      self.ready = false
      return
    }
    ...
    self.storePromise = new Promise(function (resolve, reject) {
      ...
      const extensions = atom.config.get('atom-notes-tweak.extensions')
      ...
      store.on('ready', () => {
        ...
        console.log(`atom-notes-tweak: ready in ${td}ms`)
      })
      store.initialize()
      resolve(store)
    })
...
function handleEvents (self) {
  ...
  // user commands
  self.subs.add(
    atom.commands.add('atom-workspace', 'atom-notes-tweak:toggle', () => self.getNotesView().toggle()),
    atom.commands.add('atom-workspace', 'atom-notes-tweak:interlink', () => openInterlink())
  )
...
function ensureNotesDirectory () {
  ...
  let defaultNotesDirectory = path.join(packagesDirectory, 'atom-notes-tweak', 'notebook')
...
function autosave (paneItem) {
  if (!atom.config.get('atom-notes-tweak.enableAutosave')) return
